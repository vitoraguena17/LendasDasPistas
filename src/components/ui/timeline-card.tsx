  ...
  ({ alignment, period, title, text, imageSrc, priority = false }, ref) => {
    ...
    return (
      <div ref={ref} className="relative flex w-full justify-between items-center pl-16 md:pl-0 timeline-card">
        ...
        {/* CARDS COM SOMBRA MARCANTE:
            Substituído por shadow-[0_20px_40px_rgba(0,0,0,0.06)] que projeta um efeito de elevação real e visível no fundo claro */}
        <div className={`w-full md:w-[45%] bg-white/70 backdrop-blur-xl p-8 rounded-3xl border border-zinc-200 group card-content shadow-[0_20px_40px_rgba(0,0,0,0.06)] ${alignment === 'right' ? 'md:order-2' : 'md:order-1'}`}>
        ...
        {imageSrc ? (
          /* IMAGENS COM PROJEÇÃO DE SOMBRA:
             Colocamos a sombra na div pai externa e removemos o overflow do container de sombra para o navegador não cortar os cantos projetados. 
             O efeito shadow-[0_25px_50px_-12px_rgba(0,0,0,0.25)] cria uma profundidade de galeria editorial incrível. */
