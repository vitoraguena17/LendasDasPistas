...
export function HeroSection() {
  ...
  // 1. Bloqueia o Scroll no carregamento da página
  useEffect(() => {
    if (lenis) {
      window.scrollTo(0, 0);
  ...
  // 2. ANIMAÇÃO DE ENTRADA DO BOTÃO: Muito mais fluida usando efeito Back (Amortecimento de mola)
  useGSAP(() => {
    gsap.fromTo(".gsap-hero-btn", 
      { opacity: 0, y: 40, scale: 0.95 },
      {
        ...
        duration: 1.4,
        delay: 3.6, 
        ease: "back.out(1.7)" // Dá uma leve "passada" da posição final e volta, parecendo uma mola física
      }
    );
  }, { scope: sectionRef });

  // 3. Ação do Botão: Libera o scroll e faz a transição suave
  ...
  return (
    <section ref={sectionRef} className="w-full h-dvh overflow-hidden flex flex-col justify-center relative pt-[11dvh] pb-20 md:pb-16">
      <div className="flex flex-col md:grid md:grid-cols-12 items-start w-full h-full">
        <div className="md:col-start-1 md:col-end-11 md:row-start-1 z-10 flex flex-col md:justify-between h-auto md:h-full pointer-events-none shrink-0">
          ...
          <div className="mt-4 md:mt-0 max-w-[95%] sm:max-w-65 lg:max-w-85 md:mb-4 lg:mb-8 pointer-events-auto flex flex-col items-start gap-8">
            ...
            {/* HOVER TOTALMENTE REFORMULADO:
                - Trocada a cor base para bg-zinc-900 para dar melhor contraste.
                - Ao passar o mouse, ele inverte de forma sutil mudando background, texto e bordas ao mesmo tempo.
                - duration-500 garante que a troca de cor seja sedosa e elegante. */}
            <button
            ...
            >
              {/* Indicador luminoso que muda de cor de acordo com o fundo */}
              ...
              {/* ÍCONE COM FLUIDEZ: O group-hover aplica o deslocamento vertical na seta simultaneamente com o fade de cores */}
              <svg 
                width="14" 
                height="14" 
                viewBox="0 0 24 24" 
      ...
      <div className="absolute bottom-6 md:bottom-8 left-1/2 -translate-x-1/2 z-30 opacity-0 pointer-events-none">
        <ScrollIndicator />
      </div>
    </section>
  );
